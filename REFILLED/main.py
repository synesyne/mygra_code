...
from networks import resnet, wide_resnet, mobile_net, vit
...
if __name__ == '__main__':
    ...
    # task arguments
    parser.add_argument('--data_name', type=str, default='CIFAR-100', choices=['CIFAR-100', 'CUB-200', 'CUB-l100'])
    parser.add_argument('--teacher_network_name', type=str, default='wide_resnet', choices=['resnet', 'wide_resnet', 'mobile_net', 'vit'])
    parser.add_argument('--student_network_name', type=str, default='wide_resnet', choices=['resnet', 'wide_resnet', 'mobile_net', 'vit'])
    # experiment environment arguments
    parser.add_argument('--devices', type=int, nargs='+', default=[0]) # GV.DEVICES)
    ...
    parser.add_argument('--n_workers', type=int, default=4) # GV.WORKERS)
    ...
    parser.add_argument('--n_training_epochs1', type=int, default=2)#0)
    parser.add_argument('--n_training_epochs2', type=int, default=2)#0)
    ...
    # if the model is vit, the arguments are:
    parser.add_argument('--vit_image_size', type=int, default=32)
    parser.add_argument('--vit_patch_size', type=int, default=4)
    parser.add_argument('--vit_num_classes', type=int, default=100)
    parser.add_argument('--vit_dim', type=int, default=512)
    parser.add_argument('--vit_depth', type=int, default=6) #1)
    parser.add_argument('--vit_heads', type=int, default=8) #1)
    parser.add_argument('--vit_mlp_dim', type=int, default=1024)
    parser.add_argument('--vit_channels', type=int, default=3)
    ...
    if args.teacher_network_name == 'resnet':
        ...
        teacher_args = copy.copy(args)
        # 因为我看到pretrain.py中我使用的是默认值，就是16和1，与main.py中默认值相同，所以不更改，将这一行注释掉
    ...
    # 添加vit的代码：
    elif args.teacher_network_name == 'vit':
        teacher_args = copy.copy(args)
        # teacher_args.ca = 1.0
        teacher = vit.MyNetwork(teacher_args)
        pretrained_teacher_save_path = 'saves/pretrained_teachers/' + args.data_name + '_vit_teacher.model'    
